...
exports.getVisual = async (req, res, next) => {
  ...
  return Visual.findAll({
    ...
    attributes: [
      ...
      'creator',
      'VisualId',
      [sequelize.fn('ST_Envelope', sequelize.col('geom')), 'extent'],
    ...
    visuals.forEach((v) => {
      ...
      if (docs.length) {
        visual.documents = docs.map((d) => {
          const doc = d;
          doc.extent = [doc.extent.coordinates[0][0], doc.extent.coordinates[0][2]];
          ...
          return _.omit(doc, 'VisualId', 'remoteid', 'x', 'y');
        });
